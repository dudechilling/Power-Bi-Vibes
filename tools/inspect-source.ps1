...
function Get-CsvSchema {
    ...
    $result.structures += [ordered]@{
        ...
        kind = 'delimited_file'
        columns = @($headers | ForEach-Object { [ordered]@{ name = $_; declared_type = 'unknown' } })
    }
    return $result
}
...
function Get-XlsxCellText {
    ...
    if ($cellType -eq 's') {
        $index = 0
        if ([int]::TryParse($raw, [ref]$index) -and $index -ge 0 -and $index -lt $SharedStrings.Count) { return [string]$SharedStrings[$index] }
    }
    return $raw
}
...
function Get-XlsxSchema {
    ...
    try {
        ...
        if ($null -eq $workbookEntry -or $null -eq $relsEntry) { throw 'The XLSX file is missing required workbook metadata.' }
        $wbStream = $workbookEntry.Open(); $relsStream = $relsEntry.Open(); $wbReader = $null; $relsReader = $null
        try {
            $wbReader = New-Object System.IO.StreamReader($wbStream)
            $relsReader = New-Object System.IO.StreamReader($relsStream)
            [xml]$workbook = $wbReader.ReadToEnd(); [xml]$rels = $relsReader.ReadToEnd()
        } finally {
            if ($wbReader) { $wbReader.Dispose() }; if ($relsReader) { $relsReader.Dispose() }
            $wbStream.Dispose(); $relsStream.Dispose()
        }
        ...
        foreach ($sheet in $workbook.SelectNodes('//x:sheets/x:sheet', $wbNs)) {
            ...
            if ($target.StartsWith('/')) { $entryPath = $target.TrimStart('/') } elseif ($target.StartsWith('xl/')) { $entryPath = $target } else { $entryPath = 'xl/' + $target.TrimStart('./') }
            $sheetEntry = $archive.GetEntry($entryPath)
            if ($null -eq $sheetEntry) { $result.warnings += "Could not inspect worksheet '$sheetName' ($entryPath)."; continue }
            $sheetStream = $sheetEntry.Open(); $sheetReader = $null
            try { $sheetReader = New-Object System.IO.StreamReader($sheetStream); [xml]$sheetXml = $sheetReader.ReadToEnd() } finally { if ($sheetReader) { $sheetReader.Dispose() }; $sheetStream.Dispose() }
            $sheetNs = New-Object System.Xml.XmlNamespaceManager($sheetXml.NameTable)
            $sheetNs.AddNamespace('x', 'http://schemas.openxmlformats.org/spreadsheetml/2006/main')
            $headerRow = $null
            foreach ($rowNode in $sheetXml.SelectNodes('//x:sheetData/x:row', $sheetNs)) { if ($rowNode.SelectNodes('./x:c', $sheetNs).Count -gt 0) { $headerRow = $rowNode; break } }
            $headers = @()
            if ($null -ne $headerRow) {
                foreach ($cell in $headerRow.SelectNodes('./x:c', $sheetNs)) {
                    ...
                    $headers += $(if ([string]::IsNullOrWhiteSpace($text)) { '<blank-header>' } else { $text })
                }
            }
            $result.structures += [ordered]@{
                name = $sheetName
        ...
        foreach ($tableEntry in @($archive.Entries | Where-Object { $_.FullName -like 'xl/tables/*.xml' })) {
            $tableStream = $tableEntry.Open(); $tableReader = $null
            try { $tableReader = New-Object System.IO.StreamReader($tableStream); [xml]$tableXml = $tableReader.ReadToEnd() } finally { if ($tableReader) { $tableReader.Dispose() }; $tableStream.Dispose() }
            ...
            if ([string]::IsNullOrWhiteSpace($tableName)) { $tableName = [string]$tableNode.GetAttribute('name') }
            $tableColumns = @($tableXml.SelectNodes('//x:tableColumns/x:tableColumn', $tableNs) | ForEach-Object { [ordered]@{ name = [string]$_.GetAttribute('name'); declared_type = 'unknown' } })
            $result.structures += [ordered]@{ name = $tableName; kind = 'excel_table'; columns = $tableColumns }
        }
...
function Invoke-SqliteJson {
    ...
    if ($null -eq $sqlite) { throw 'SQLite inspection requires sqlite3.exe on PATH. Install SQLite CLI locally or use a scrubbed schema description instead.' }
    $output = & $sqlite.Source -json $ResolvedPath $Sql
    if ($LASTEXITCODE -ne 0) { throw 'sqlite3 failed while inspecting schema.' }
...
function Get-SqliteSchema {
    ...
    foreach ($obj in $objects) {
        $name = [string]$obj.name; $safeName = $name.Replace("'", "''")
        $columns = @(Invoke-SqliteJson -ResolvedPath $ResolvedPath -Sql "PRAGMA table_info('$safeName');")
        $result.structures += [ordered]@{
            name = $name; kind = [string]$obj.type
            columns = @($columns | ForEach-Object { [ordered]@{ name = [string]$_.name; declared_type = [string]$_.type; not_null = ([int]$_.notnull -eq 1); primary_key_order = [int]$_.pk } })
        }
    }
    return $result
}
...
if (-not (Test-Path -LiteralPath $resolvedPath -PathType Leaf)) { throw 'Path must refer to a file.' }
...
if ([string]::IsNullOrWhiteSpace($OutputPath)) { $json } else {
