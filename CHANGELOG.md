...
- Hardened `tools/inspect-source.ps1` with explicit XLSX `-HeaderRow` selection, suspicious-header warnings that expose only row/count metadata, and a `-Delimiter` override for delimited sources; added a Windows PowerShell fixture suite covering normal/banner/merged-spacer XLSX, comma/tab/semicolon files, privacy of data-row values, and SQLite when available.
- Added a mandatory post-mutation **Local QA handoff** for client repositories: final agent responses now include a safe copy/paste PowerShell block that checks local Git state, pulls with `git pull --ff-only`, starts only repository-defined services when applicable, launches the relevant Power BI entrypoint when known, and follows with task-specific QA actions and expected results. Integrity checks now guard this response contract.
